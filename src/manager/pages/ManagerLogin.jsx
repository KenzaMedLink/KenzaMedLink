...
export default function ManagerLogin() {
  ...
  const { login, logout, initializing, profile } = useAuth();
  ...
  useEffect(() => {
    if (!initializing && profile && ["manager", "owner"].includes(profile.role)) {
  ...
  const handleLogin = async (e) => {
    ...
    if (!["manager", "owner"].includes(role)) {
      setError("Not authorized for manager portal.");
  ...
  return (
    <div className="h-[100dvh] flex items-center justify-center bg-gradient-to-br from-gray-50 to-gray-100 px-4">
      {/* ✅ increased height/space: p-6 + more spacing */}
      <div className="w-full max-w-md bg-white rounded-2xl shadow-xl border border-gray-100 p-6">
        ...
        <div className="text-center mb-4">
          ...
          <p className="text-gray-500 text-sm mt-1">Manager Portal Login</p>
        </div>
